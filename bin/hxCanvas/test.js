...
var cornerContour_web_CanvasPlus = function(me,x,y) {
	if(y == null) {
		y = 0;
	}
	if(x == null) {
...
cornerContour_web_CanvasPlus.__name__ = true;
var cornerContourSamples_hxCanvas_CanvasSetup = function() {
	...
	var this1 = new cornerContour_web_CanvasPlus(canvas.getContext("2d",null),10,10);
	this.surface = this1;
	var factor = 4.;
	this.overSampleCanvas(canvas,this.surface.me,factor);
};
cornerContourSamples_hxCanvas_CanvasSetup.__name__ = true;
cornerContourSamples_hxCanvas_CanvasSetup.prototype = {
...
var cornerContourSamples_hxCanvas_SvgExample = function() {
	this.canvasSetup = new cornerContourSamples_hxCanvas_CanvasSetup();
	haxe_Log.trace("Svg example on Canvas",{ fileName : "src/cornerContourSamples/hxCanvas/SvgExample.hx", lineNumber : 13, className : "cornerContourSamples.hxCanvas.SvgExample", methodName : "new"});
	var pen = new cornerContour_Pen2D(-16776961);
	var g = this.canvasSetup.surface;
	cornerContourSamples_svg_All_svgs(pen);
	var pen1 = pen;
	var data = pen1.arr;
	...
	while(_g < _g1) {
		var i = _g++;
		pen1.arr[0] = i;
		var alpha = ((cornerContour_io_Array2DTriangles.get_color(data) | 0) >> 24 & 255) / 255 - 0.000000000000001;
		...
		var color = (cornerContour_io_Array2DTriangles.get_color(data) | 0) << 8 >> 8;
		var r = color >> 16 & 255;
		var g1 = color >> 8 & 255;
		var b = color & 255;
		var a = ((cornerContour_io_Array2DTriangles.get_color(data) | 0) >> 24 & 255) / 255 - 0.000000000000001;
		g.me.fillStyle = "rgba(" + r + "," + g1 + "," + b + "," + a + ")";
		g.me.beginPath();
		g.me.lineWidth = 0.0000000000000001;
		var r1 = color >> 16 & 255;
		var g2 = color >> 8 & 255;
		var b1 = color & 255;
		var a1 = 0.0;
		g.me.strokeStyle = "rgba(" + r1 + "," + g2 + "," + b1 + "," + a1 + ")";
		g.x = ax;
		g.y = ay;
		g.me.moveTo(ax,ay);
		g.x = bx;
...
cornerContourSamples_hxCanvas_SvgExample.__name__ = true;
function cornerContourSamples_hxCanvas_SvgExample_main() {
	new cornerContourSamples_hxCanvas_SvgExample();
}
function cornerContourSamples_svg_All_svgs(pen2D) {
	cornerContourSamples_svg_Ellipses_arcSvg(pen2D);
	pen2D.currentColor = -16776961;
	cornerContourSamples_svg_Kiwi_kiwiSvg(pen2D);
	cornerContourSamples_svg_Curves_cubicSvg(pen2D);
	cornerContourSamples_svg_Curves_quadSvg(pen2D);
}
function cornerContourSamples_svg_Curves_cubicSvg(pen2D,width) {
	if(width == null) {
		width = 10;
	}
	var sketcher = new cornerContour_Sketcher(pen2D,4,3);
	...
	p.parse(cornerContourSamples_svg_Curves_cubictest_d);
}
function cornerContourSamples_svg_Curves_quadSvg(pen2D,width) {
	if(width == null) {
		width = 1.;
	}
	var sketcher = new cornerContour_Sketcher(pen2D,4,3);
	...
	var p = new justPath_SvgPath(translateContext);
	p.parse(cornerContourSamples_svg_Curves_quadtest_d);
}
var cornerContourSamples_svg_Ellipses = function(pen2D_) {
...
cornerContourSamples_svg_Ellipses.__name__ = true;
cornerContourSamples_svg_Ellipses.prototype = {
	draw: function() {
...
function cornerContourSamples_svg_Ellipses_arcSvg(pen2D) {
	var ellipses = new cornerContourSamples_svg_Ellipses(pen2D);
	ellipses.draw();
}
function cornerContourSamples_svg_Kiwi_kiwiSvg(pen2D) {
	var sketcher = new cornerContour_Sketcher(pen2D,4,3);
	sketcher.width = 2;
	...
	p.parse(cornerContourSamples_svg_Kiwi_kiwi_d);
}
